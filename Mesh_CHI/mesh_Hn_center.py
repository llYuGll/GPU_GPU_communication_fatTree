...
# FIXED: Updated node assignment constants for center HN placement
N_HOSTS = 16 # Total IPs: 14 GPUs + 1 CPU + 1 Global Memory
N_GPUS = 14 # GPU nodes (all except CPU and HN)
MESH_SIZE = 4 # 4x4 mesh for 16 nodes
CPU_NODE_ID = 14 # CPU (inactive)
GLOBAL_MEMORY_ID = 5 # FIXED: Global Memory (HOME NODE) now at center position (row=1, col=1)
...
print(f" - FIXED: Global Memory (HN) at center position node {GLOBAL_MEMORY_ID}")
...
class MeshNetwork:
    def __init__(self, env, visualizer=None):
        ...
        LOGGER.log(f" Home Node: Global Memory (ID {GLOBAL_MEMORY_ID}) at CENTER position")
    ...
    def _build_mesh_topology(self):
        ...
        # FIXED: Updated node creation logic to support flexible HN placement
        for i in range(N_HOSTS):
            # FIXED: Determine node type using new flexible assignment
            if i == GLOBAL_MEMORY_ID:
...
# FIXED: Traffic Generator - Updated to work with flexible GPU assignment
class RealisticTrafficGenerator:
    ...
    def generate_operation(self):
        ...
        # FIXED: Only GPU nodes can initiate operations (not CPU or Global Memory)
        # Get all GPU node IDs (exclude CPU and HN)
        gpu_nodes = [i for i in range(N_HOSTS) if i not in (CPU_NODE_ID, GLOBAL_MEMORY_ID)]
    ...
    def export_parameters(self, filename=None):
        ...
        try:
            with open(filename, 'w', encoding='utf-8') as f:
                ...
                f.write(f"Global Memory (HN) Position: Node {GLOBAL_MEMORY_ID} (CENTER)\n")
...
# FIXED: Proper Mesh Visualizer with realistic packet animation through routers
class MeshVisualizer:
    ...
    def _draw_topology(self):
        ...
        self.main_ax.set_title("CHI 2D Mesh NoC [CENTER HN PLACEMENT]",
        ...
        # FIXED: Draw nodes with updated node type checking
        for i, pos in self.node_positions.items():
            if i == GLOBAL_MEMORY_ID:
                # FIXED: Global Memory (Home Node) - now at center position
                ...
                # FIXED: GPU nodes (all others except CPU and HN)
                rect = patches.Rectangle((pos[0]-0.3, pos[1]-0.3), 0.6, 0.6,
    ...
    def create_cache_window(self):
        ...
        if self.cache_window is None:
            ...
            self.cache_window.title("CHI Mesh with CENTER HN: Memory states visualization")
    ...
    def _setup_cache_display(self):
        ...
        self.cache_ax.set_title("Memory states visualization - CENTER HN PLACEMENT",
                              fontsize=16, fontweight='bold', pad=20)

        # FIXED: Setup grid with proper space utilization - account for flexible GPU count
        gpu_count = len([i for i in range(N_HOSTS) if i not in (CPU_NODE_ID, GLOBAL_MEMORY_ID)])
        self.cache_ax.set_xlim(-1.2, gpu_count + 3.8)
        ...
        # FIXED: Labels with proper spacing for flexible GPU assignment
        gpu_nodes = [i for i in range(N_HOSTS) if i not in (CPU_NODE_ID, GLOBAL_MEMORY_ID)]
        x_labels = [f"GPU{i}" for i in gpu_nodes] + ["GlobalMem"]
        x_positions = list(range(len(gpu_nodes))) + [len(gpu_nodes) + 2] # 2-unit gap before memory
    ...
    def update_cache_display(self, network):
        """CHI-Mesh NoC Simulator with CENTER HN PLACEMENT"""
        if self.cache_ax is None:
            return
        ...
        # FIXED: Display GPU caches with LRU ordering for flexible GPU assignment
        gpu_nodes = [i for i in range(N_HOSTS) if i not in (CPU_NODE_ID, GLOBAL_MEMORY_ID)]
        for gpu_index, gpu_id in enumerate(gpu_nodes):
            ...
            for addr in range(GLOBAL_MEMORY_SIZE):
                ...
                rect = patches.Rectangle((gpu_index - 0.48, addr - 0.45), 0.96, 0.90,
                ...
                self.cache_ax.text(gpu_index, addr, text, ha='center', va='center',
        ...
        x_pos = len(gpu_nodes) + 2 # 2-unit gap from last GPU
...
print("FIXED: Mesh visualizer with center HN placement and flexible GPU assignment")
...
def run_mesh_noc_simulation(command_queue, network, traffic_generator):
    ...
    LOGGER.log("FIXED CHI Protocol Mesh NoC Simulation with CENTER HN PLACEMENT")
    LOGGER.log(f"Home Node: Global Memory (ID {GLOBAL_MEMORY_ID}) at CENTER position")
    gpu_nodes = [i for i in range(N_HOSTS) if i not in (CPU_NODE_ID, GLOBAL_MEMORY_ID)]
    LOGGER.log(f"GPU nodes: {gpu_nodes} (all request nodes)")
    ...
    while True:
        try:
            ...
            try:
                ...
                # FIXED: Only GPU nodes can initiate operations
                gpu_nodes = [i for i in range(N_HOSTS) if i not in (CPU_NODE_ID, GLOBAL_MEMORY_ID)]
                if node_id not in gpu_nodes:
                    LOGGER.log(f"Only GPU nodes {gpu_nodes} can initiate operations")
                    continue
...
print("Updated simulation controller for mesh topology with center HN")

# FIXED: Main GUI Application for CHI Protocol Mesh NoC with CENTER HN
class CorrectedCHIMeshNoCApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("CHI Protocol Mesh NoC with CENTER HN PLACEMENT")
        ...
        self.network = MeshNetwork(self.env, self.visualizer)
        ...
        gpu_nodes = [i for i in range(N_HOSTS) if i not in (CPU_NODE_ID, GLOBAL_MEMORY_ID)]
        LOGGER.log("CHI-Mesh NoC Simulator with CENTER HN PLACEMENT")
        LOGGER.log("=" * 60)
        LOGGER.log("CHI PROTOCOL")
        LOGGER.log(f" Home Node: Global Memory (ID {GLOBAL_MEMORY_ID}) at CENTER position")
        LOGGER.log(f" GPU Nodes: {gpu_nodes} (request nodes)")
    ...
    def _create_gui(self):
        ...
        viz_panel = ttk.LabelFrame(self, text="CHI-Mesh NoC with CENTER HN", padding=5)
    ...
    def _create_quick_actions(self, parent):
        ...
        quick_frame = ttk.LabelFrame(parent, text="Quick Actions (Test CENTER HN with Flexible GPUs)", padding=5)
        quick_frame.grid(row=1, column=0, sticky="ew", pady=(0, 5))

        # FIXED: Update actions to work with flexible GPU assignment
        gpu_nodes = [i for i in range(N_HOSTS) if i not in (CPU_NODE_ID, GLOBAL_MEMORY_ID)]
        first_gpu = gpu_nodes[0] if gpu_nodes else 0
        second_gpu = gpu_nodes[1] if len(gpu_nodes) > 1 else first_gpu

        actions = [
            (f"Read GPU{first_gpu} A0", f"read {first_gpu} 0"),
            (f"Single Read (→E)", f"read {first_gpu} 12"),
    ...
    def _create_cache_controls(self, parent):
        ...
        cache_frame = ttk.LabelFrame(parent, text="CHI Cache View (CENTER HN)", padding=5)
    ...
    def _create_extract_panel(self, parent):
        ...
        # Enhanced info text
        info_text = f"""CENTER HN Configuration:
• HN at node {GLOBAL_MEMORY_ID} (center position)
• {len([i for i in range(N_HOSTS) if i not in (CPU_NODE_ID, GLOBAL_MEMORY_ID)])} flexible GPU nodes

Realistic On-Chip NoC Timing:
...
• Link utilization estimates"""
    ...
    def _initialize_simulation(self):
        ...
        self.simulation_thread = threading.Thread(
            target=run_mesh_noc_simulation,
    ...
    def show_performance_summary(self):
        ...
        try:
            if PERF_TRACKER:
                ...
                summary_window.title("Enhanced Performance Summary - CENTER HN")
...
Topology: 2D Mesh with CENTER HN placement
Global Memory (HN) Position: Node {GLOBAL_MEMORY_ID} (center)
...
"""
    ...
    def run_modified_writeback_test(self):
        ...
        # FIXED: Use flexible GPU assignment
        gpu_nodes = [i for i in range(N_HOSTS) if i not in (CPU_NODE_ID, GLOBAL_MEMORY_ID)]
        gpu0 = gpu_nodes[0] if gpu_nodes else 0
        gpu1 = gpu_nodes[1] if len(gpu_nodes) > 1 else gpu0

        LOGGER.log("Testing ENHANCED writeback behavior with CENTER HN...")
        LOGGER.log(f" Step 1: GPU{gpu0} writes A8=150 (gets dirty)")
        self.queue_command(f"write {gpu0} 8 150")

        self.after(2000, lambda: [
            LOGGER.log(f" Step 2: GPU{gpu1} writes A8=250 (should invalidate GPU{gpu0} WITHOUT writeback)"),
            LOGGER.log(f" Because GPU{gpu1} will now have dirty data, GPU{gpu0} writeback is skipped"),
            self.queue_command(f"write {gpu1} 8 250")
        ])

        self.after(4000, lambda: [
            LOGGER.log(" Step 3: Check memory - should show STALE (GPU OWNS), no specific GPU tracked"),
            LOGGER.log(f" GPU{gpu1} now has dirty A8=250, GPU{gpu0} was invalidated without writeback"),
    ...
    def run_exclusive_to_shared_test(self):
        ...
        LOGGER.log("Testing ENHANCED Exclusive (E) to Shared (S) transition with CENTER HN...")
        LOGGER.log(f" Step 1: GPU{gpu0} reads A15 (should get EXCLUSIVE - green color)")
        self.queue_command(f"read {gpu0} 15")

        self.after(2000, lambda: [
            LOGGER.log(f" Step 2: Check cache window - GPU{gpu0} should have A15 in EXCLUSIVE (E) green state"),
            LOGGER.log(" This shows single reader gets exclusive clean access")
        ])

        self.after(4000, lambda: [
            LOGGER.log(f" Step 3: GPU{gpu1} also reads A15 (should trigger E→S transition)"),
            self.queue_command(f"read {gpu1} 15")
        ])

        self.after(6000, lambda: [
            LOGGER.log(f" Step 4: Check cache window - BOTH GPUs should now have A15 in SHARED (S) blue state"),
    ...
    def run_coherency_test(self):
        ...
        gpu0 = gpu_nodes[0] if gpu_nodes else 0
        gpu1 = gpu_nodes[1] if len(gpu_nodes) > 1 else gpu0
        gpu2 = gpu_nodes[2] if len(gpu_nodes) > 2 else gpu0

        LOGGER.log("Testing cache coherency with performance tracking and CENTER HN...")
        self.queue_command(f"read {gpu0} 7") # GPU0 reads A7
        self.after(1000, lambda: self.queue_command(f"read {gpu1} 7")) # GPU1 reads A7 (should share)
        self.after(2000, lambda: self.queue_command(f"write {gpu2} 7 200")) # GPU2 writes A7 (invalidates others)
        self.after(3000, lambda: self.queue_command(f"read {gpu0} 7")) # GPU0 reads A7 (should get new value)
    ...
    def run_eviction_test(self):
        ...
        LOGGER.log("Testing LRU evictions with performance tracking and CENTER HN...")
        LOGGER.log("Writing to 5 addresses to force evictions...")

        # Fill cache completely then add one more to force eviction
        self.queue_command(f"write {gpu0} 0 10")
        self.after(500, lambda: self.queue_command(f"write {gpu0} 1 20"))
        self.after(1000, lambda: self.queue_command(f"write {gpu0} 2 30"))
        self.after(1500, lambda: self.queue_command(f"write {gpu0} 3 40"))
        self.after(2000, lambda: [
            LOGGER.log(" Cache full (4 slots), next write will evict oldest"),
            self.queue_command(f"write {gpu0} 4 50")
        ])
    ...
    def update_log_display(self):
        ...
        for message in messages:
            ...
            elif "Testing" in message or "Step" in message or "ENHANCED" in message or "CENTER" in message:
