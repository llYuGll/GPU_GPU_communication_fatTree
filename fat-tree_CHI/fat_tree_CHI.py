...
# Configuration - Based on Fat-Tree Documentation
...
ACCESS_LINK_BW = 200e9 # 200 Gbps GPU to Leaf (on-chip realistic)
BACKBONE_LINK_BW = 400e9 # 400 Gbps Leaf to Spine (on-chip realistic) 
SWITCH_PROCESSING_DELAY = 3e-9 # 50ns per switch hop (on-chip realistic)
BUFFER_SIZE = 16 # Switch buffer size (packets)
...
# PROPER Fat-Tree Switch with routing logic
class FatTreeSwitch:
    def __init__(self, env, switch_id, level, switch_type):
        ...
        self.total_queue_delay = 0.0  # NEW: Track total queuing delay
        self.queue_delay_samples = 0  # NEW: Count of delay samples
    ...
    def enqueue_packet_to_port(self, port, packet):
        """Public method to push packet into a switch input port (called by previous hop)."""
        self._ensure_port_exists(port)
        queue_store = self.input_queues[port]

        # simulate finite buffer size -> if too many waiting, drop (or block with backpressure)
    ...
    def _port_service_loop(self, port):
        ...
        while True:
            try:
                ...
                # compute serialization/service time for this hop based on destination
                output_port = self.route_packet(packet)

                # pick link bandwidth based on if next hop is switch or node
                ...
                # total service = switch processing + serialization
                service_time = SWITCH_PROCESSING_DELAY + link_ser

                # simulate service (this models both switch pipeline and link serialization/transfer)
                yield self.env.timeout(service_time)

                # record service delay
                packet.switch_delays.append(('service', self.switch_id, service_time))

                # forward: if next is a node -> deliver; if switch -> append to its input port
                if output_port in self.connected_nodes:
                    node_id = self.connected_nodes[output_port]
                    # Final delivery to node - use the network's delivery system
                    ...
                    # map output_port->their input_port (simplified mapping)
                    next_input_port = self._map_to_input_port(output_port, next_switch)
                    # put into next switch via its enqueue_packet_to_port method
                    success = next_switch.enqueue_packet_to_port(next_input_port, packet)
                    if not success:
                        # Handle backpressure - packet was dropped at next switch
                        LOGGER.log(f"Switch {self.switch_id} -> {next_switch.switch_id}: packet dropped due to congestion")
    ...
    def _map_to_input_port(self, output_port, next_switch):
        """Map output port to input port of next switch"""
        # For Fat-Tree: leaf->spine mapping and spine->leaf mapping
        if self.level == 0 and next_switch.level == 1:  # leaf to spine
            return self._get_leaf_id()  # Use leaf index as spine input port
        elif self.level == 1 and next_switch.level == 0:  # spine to leaf
            return 4  # Spines connect to leaf ports 4,5
        else:
            return 0  # Default

    def _get_leaf_id(self):
        """Get leaf switch ID from switch_id string"""
        try:
    ...
    def connect_node(self, port, node_id):
        """Connect a node to this switch port"""
        self.connected_nodes[port] = node_id

    def connect_switch(self, port, switch):
        """Connect another switch to this port"""
        self.connected_switches[port] = switch

    def _build_routing_table(self):
        """Build routing table based on Fat-Tree topology"""
        if self.level == 0: # Leaf switch
            # Route to directly connected nodes
            for port, node_id in self.connected_nodes.items():
                self.routing_table[node_id] = port

            # Route to nodes on other leaves via spine switches
            for node_id in range(N_HOSTS):
                if node_id not in self.routing_table:
                    # Route to spine (use round-robin)
                    spine_port = 4 + (node_id % N_SPINE) # Ports 4,5 for spine
                    self.routing_table[node_id] = spine_port

        elif self.level == 1: # Spine switch
            # Route to all nodes via appropriate leaf
            for node_id in range(N_HOSTS):
    ...
    def route_packet(self, packet):
        ...
        if dest_id in self.routing_table:
            output_port = self.routing_table[dest_id]
            return output_port
        else:
            # Default routing - send up to spine or down to leaf
            if self.level == 0: # Leaf - send to first spine
                return 4
            else: # Spine - send to first leaf
                return 0

    def _deliver_to_node(self, packet, node_id):
        """Deliver packet to final destination node"""
        # Add final delivery delay
        yield self.env.timeout(3e-9) # 50ns delivery delay (on-chip realistic)

        # Signal that this packet is ready for final delivery
        # The network will handle the actual delivery to the node
...
print(f" - Switch processing: {SWITCH_PROCESSING_DELAY*1e9:.1f}ns (was 1ms)")
print(f" - Access links: {ACCESS_LINK_BW/1e9:.0f} Gbps (was 10 Gbps)")
print(f" - Backbone links: {BACKBONE_LINK_BW/1e9:.0f} Gbps (was 40 Gbps)")
print(f" - Delivery delay: 50ns (was 0.5ms)")
...
# PROPER 2-Level Fat-Tree Network with MINIMAL INSTRUMENTATION
class ProperFatTreeNetwork:
    ...
    def _build_fat_tree_topology(self):
        ...
        for i in range(N_HOSTS):
            ...
            # Connect node to appropriate leaf switch
            leaf_idx = i // LEAF_HOSTS
            port = i % LEAF_HOSTS
            self.leaf_switches[leaf_idx].connect_node(port, i)
            LOGGER.log(f"Connected {node.role}{i} to leaf_{leaf_idx} port {port}")
        ...
        for leaf_idx, leaf in enumerate(self.leaf_switches):
            for spine_idx, spine in enumerate(self.spine_switches):
                # Each leaf connects to all spines (full connectivity)
                leaf.connect_switch(4 + spine_idx, spine) # Ports 4,5 for spines
                spine.connect_switch(leaf_idx, leaf) # Ports 0,1,2,3 for leaves

        # 5. Build routing tables for all switches
        for leaf in self.leaf_switches:
            leaf._build_routing_table()
        for spine in self.spine_switches:
            spine._build_routing_table()

        LOGGER.log("Fat-Tree topology built with proper switch connections")

    def _route_packet(self, packet):
        """Route packet through Fat-Tree network with realistic delays"""
    ...
    def _route_packet_through_switches(self, packet):
        """Route packet through actual switches with proper delays and MINIMAL INSTRUMENTATION"""
        source_id = packet.source_id
        dest_id = packet.dest_id
        ...
        # Determine routing path through switches
        source_leaf_idx = source_id // LEAF_HOSTS
        dest_leaf_idx = dest_id // LEAF_HOSTS

        total_delay = 0.0

        # Stage 1: Source to source leaf switch
        ...
        # Stage 2: Route through switch hierarchy
        if source_leaf_idx == dest_leaf_idx:
            # Same leaf switch - direct routing
            ...
            # Different leaf switches - route through spine
            # Source leaf to spine
            spine_idx = self._select_spine_switch(packet)
            yield self.env.timeout(SWITCH_PROCESSING_DELAY) # Leaf processing
            yield self.env.timeout(self._get_backbone_link_delay()) # Leaf to spine
            ...
            yield self.env.timeout(SWITCH_PROCESSING_DELAY) # Spine processing
            yield self.env.timeout(self._get_backbone_link_delay()) # Spine to leaf
            total_delay += SWITCH_PROCESSING_DELAY + self._get_backbone_link_delay()
            packet.add_hop(f"leaf_{dest_leaf_idx}")

            # Destination leaf processing
            yield self.env.timeout(SWITCH_PROCESSING_DELAY)
            total_delay += SWITCH_PROCESSING_DELAY
            packet.add_hop(f"leaf_{dest_leaf_idx}_out")

        # Stage 3: Destination leaf to destination node
        yield self.env.timeout(self._get_access_link_delay())
        total_delay += self._get_access_link_delay()
        packet.add_hop(f"IP_{dest_id}")

        # MINIMAL INSTRUMENTATION: Record packet metrics
    ...
    def _select_spine_switch(self, packet):
        """Select spine switch for load balancing"""
        # Simple round-robin based on source/dest hash
        return (packet.source_id + packet.dest_id) % N_SPINE

    def _get_access_link_delay(self):
        """Calculate access link delay (IP to leaf)"""
        return (PACKET_SIZE * 8) / ACCESS_LINK_BW

    def _get_backbone_link_delay(self):
        """Calculate backbone link delay (leaf to spine)"""
...
class PerformanceTracker:
    ...
    def compute_summary(self, sim_time_seconds):
        ...
        backbone_capacity_bps = BACKBONE_LINK_BW / 8
        ...
        backbone_utilization = throughput_bytes_per_sec / backbone_capacity_bps
        ...
        return {
            ...
            'backbone_link_utilization': backbone_utilization,
    ...
    def print_summary(self, sim_time_seconds):
        ...
        LOGGER.log(f"Backbone util. (approx): {s['backbone_link_utilization']*100:.4f}%")
    ...
    def export_parameters(self, filename=None):
        ...
        try:
            with open(filename, 'w', encoding='utf-8') as f:
                f.write("CHI Fat-Tree NoC Performance Parameters\n")
                f.write("=" * 45 + "\n\n")
                ...
                f.write(f"Backbone Link Utilization: {summary['backbone_link_utilization']*100:.4f}%\n\n")
                ...
                f.write("-" * 21 + "\n")
                f.write(f"Topology: 2-Level Fat-Tree\n")
                f.write(f"Total Nodes: {N_HOSTS} ({N_GPUS} GPUs + 1 CPU + 1 Memory)\n")
                f.write(f"Switches: {N_LEAF} leaf + {N_SPINE} spine\n")
                f.write(f"Access Link Bandwidth: {ACCESS_LINK_BW/1e9:.0f} Gbps\n")
                f.write(f"Backbone Link Bandwidth: {BACKBONE_LINK_BW/1e9:.0f} Gbps\n")
                f.write(f"Switch Processing Delay: {SWITCH_PROCESSING_DELAY*1e9:.1f} ns\n")
...
# Proper Visualizer with realistic packet animation through switches (UNCHANGED)
class ProperFatTreeVisualizer:
    MESSAGE_COLORS = {
    ...
    def __init__(self):
        self.main_fig, self.main_ax = plt.subplots(1, 1, figsize=(16, 11)) # Taller for legend
        self.cache_fig = None
        self.cache_ax = None
    ...
    def _setup_topology(self):
        """Setup 2-level fat-tree layout with proper switch positions"""
        # Node positions (bottom level)
        self.node_positions = {}
        for i in range(N_HOSTS):
            leaf_idx = i // LEAF_HOSTS
            pos_in_leaf = i % LEAF_HOSTS
            x = leaf_idx * 6 + pos_in_leaf * 1.2 - 1.8
            y = 1
            self.node_positions[i] = (x, y)

        # Leaf switch positions (Level 1)
        self.leaf_positions = {}
        for i in range(N_LEAF):
            x = i * 6
            y = 4
        ...
        # Spine switch positions (Level 2)
        self.spine_positions = {}
        spine_spacing = (N_LEAF - 1) * 6 / (N_SPINE - 1) if N_SPINE > 1 else 0
        for i in range(N_SPINE):
            x = i * spine_spacing + 3
            y = 7
    ...
    def _draw_topology(self):
        """Draw network topology with proper Fat-Tree connections"""
        self.main_ax.clear()
        self.main_ax.set_facecolor('#F8F8FF')
        self.main_ax.set_title(" CHI 2-Level Fat-Tree NoC[BLACKMAGNET]", 
                              fontsize=14, fontweight='bold')

        # Draw backbone connections (Leaf to Spine)
        ...
        # Draw access connections (IP to Leaf)
        for ip_id, ip_pos in self.node_positions.items():
            leaf_idx = ip_id // LEAF_HOSTS
            leaf_pos = self.leaf_positions[leaf_idx]
            self.main_ax.plot([ip_pos[0], leaf_pos[0]], [ip_pos[1], leaf_pos[1]], 
                            'lightblue', linewidth=1.5, alpha=0.7)

        # Draw spine switches (Level 2)
        for spine_idx, pos in self.spine_positions.items():
            circle = patches.Circle(pos, 0.5, facecolor='red', edgecolor='darkred', linewidth=3)
            self.main_ax.add_patch(circle)
            self.main_ax.text(pos[0], pos[1], f"L2\nSpine{spine_idx}", ha='center', va='center',
                            fontsize=9, fontweight='bold', color='white')

        # Draw leaf switches (Level 1)
        for leaf_idx, pos in self.leaf_positions.items():
            circle = patches.Circle(pos, 0.4, facecolor='orange', edgecolor='darkorange', linewidth=2)
            self.main_ax.add_patch(circle)
            self.main_ax.text(pos[0], pos[1], f"L1\nLeaf{leaf_idx}", ha='center', va='center',
                            fontsize=8, fontweight='bold', color='white')

        # Draw nodes
        for ip_id, pos in self.node_positions.items():
            if ip_id < N_GPUS:
                # GPU nodes
        ...
        max_x = max(pos[0] for pos in list(self.node_positions.values()) +
                   list(self.leaf_positions.values()) + list(self.spine_positions.values()))
        self.main_ax.set_xlim(-3, max_x + 3)
        self.main_ax.set_ylim(-1.5, 9) # Extended bottom for legend
    ...
    def _setup_cache_display(self):
        ...
        self.cache_ax.set_title(" Memory states visualization",
                               fontsize=16, fontweight='bold', pad=20) # Larger title with padding

        # Setup grid with proper space utilization - no overlap
        self.cache_ax.set_xlim(-1.2, N_GPUS + 3.8) # Appropriate margins
        self.cache_ax.set_ylim(-1.5, GLOBAL_MEMORY_SIZE + 0.5) # Proper vertical space
        ...
        # Labels with proper spacing - keep GPUs at integer positions, memory separate
        ...
        self.cache_ax.set_xticklabels(x_labels, fontsize=12, rotation=45, fontweight='bold') # Larger, bold labels
        self.cache_ax.set_yticks(range(GLOBAL_MEMORY_SIZE))
        self.cache_ax.set_yticklabels([f"A{i}" for i in range(GLOBAL_MEMORY_SIZE)], fontsize=10) # Larger address labels
    ...
    def add_packet_animation(self, packet, link_name):
        """Add packet animation showing route through switches"""
        if len(packet.path) < 2:
            return

        # Create animation path through actual switches and nodes
        animation_path = []
        for i, hop in enumerate(packet.path):
            ...
            elif hop.startswith('leaf_') and not hop.endswith('_out'):
    ...
    def update_animation(self, frame):
        """Update packet animations through switch path"""
    ...
    def update_cache_display(self, network):
        """CHI-Fat-Tree NoC Simulator [BLACKMAGNET]"""
        if self.cache_ax is None:
            return
        ...
        for gpu_id in range(N_GPUS):
            ...
            for addr in range(GLOBAL_MEMORY_SIZE):
                ...
                # Adjust alpha based on LRU position (more recent = more opaque)
                alpha = 0.9 if state != 'I' else 0.3
                if lru_pos >= 0:
                    alpha = 0.5 + 0.4 * (lru_pos / max(GPU_CACHE_SIZE - 1, 1))

                rect = patches.Rectangle((gpu_id - 0.48, addr - 0.45), 0.96, 0.90, # Properly sized blocks - no overlap
                ...
                self.cache_ax.text(gpu_id, addr, text, ha='center', va='center',
                                 fontsize=8, fontweight='bold', # Appropriate font for proper sized blocks
                                 color='white' if state != 'I' else 'black')

        # Display Global Memory - CORRECTED to show writeback updates
        global_mem_node = network.nodes[GLOBAL_MEMORY_ID]
        memory_data = global_mem_node.get_memory_display_data()
        x_pos = N_GPUS + 2 # 2-unit gap from last GPU (matches x_positions)

        for addr in range(GLOBAL_MEMORY_SIZE):
            value = memory_data.get(addr, 0)

            # CORRECTED: Check if any GPU has this address in Modified state (ownership with GPU)
            has_dirty = False
            for gpu_id in range(N_GPUS):
                cache_data = network.nodes[gpu_id].get_cache_display_data()
                if addr in cache_data and cache_data[addr]['state'] == 'M':
                    has_dirty = True
                    break # Don't need to track which specific GPU - ownership is with GPU

            if has_dirty:
                # Memory has stale data - ownership is with GPU (Modified state)
                # Memory doesn't track which specific GPU owns it
                rect = patches.Rectangle((x_pos - 0.48, addr - 0.45), 0.96, 0.90, # Properly sized blocks - no overlap
                                       facecolor='#FFAAAA', edgecolor='red', linewidth=2)
                text = f"STALE\n{value}\n(GPU OWNS)"
            else:
                # Memory has current data (no dirty copies exist)
                rect = patches.Rectangle((x_pos - 0.48, addr - 0.45), 0.96, 0.90, # Properly sized blocks - no overlap
...
print("Added existing visualizer (unchanged)")

# Simulation Controller
def run_proper_noc_simulation(command_queue, network, traffic_generator):
    ...
    LOGGER.log("CORRECTED CHI Protocol Fat-Tree NoC Simulation started with ENHANCED PERFORMANCE TRACKING")
    LOGGER.log(f"Home Node: Global Memory (ID {GLOBAL_MEMORY_ID})")
    LOGGER.log(f"GPU nodes: {list(range(N_GPUS))} (all request nodes)")
    LOGGER.log("Packets route through ACTUAL switches with buffers")
...
print("Added simulation controller with enhanced performance tracking")

# Main GUI Application for CORRECTED CHI Protocol Fat-Tree NoC with MINIMAL CHANGES
class CorrectedCHIFatTreeNoCApp(tk.Tk):
    def __init__(self):
        super().__init__()

        self.title("CORRECTED CHI Protocol Fat-Tree NoC with ENHANCED PERFORMANCE TRACKING")
        self.geometry("1200x850") # Taller for legend
        self.configure(bg='#F0F0F0')

        # Initialize components
        self.env = simpy.Environment()
        self.visualizer = ProperFatTreeVisualizer()
        self.network = ProperFatTreeNetwork(self.env, self.visualizer)
        ...
        LOGGER.log(" CHI-Fat-Tree NoC Simulator [BLACKMAGNET] ")
        LOGGER.log("=" * 60)
        ...
        LOGGER.log(f" {N_LEAF} leaf + {N_SPINE} spine switches")
    ...
    def _create_gui(self):
        ...
        viz_panel = ttk.LabelFrame(self, text="CHI-Fat-Tree NoC Simulator [BLACKMAGNET]", padding=5)
    ...
    def _create_quick_actions(self, parent):
        ...
        for i, (text, command) in enumerate(actions):
            if command == "writeback_test":
                btn = ttk.Button(quick_frame, text=text, command=self.run_writeback_test)
            elif command == "modified_wb_test":
    ...
    def _create_extract_panel(self, parent):
        """Create extract panel - REPLACES CHI Protocol Info"""
...
• Switch delay: {SWITCH_PROCESSING_DELAY*1e9:.0f}ns 
• Access links: {ACCESS_LINK_BW/1e9:.0f} Gbps
• Backbone: {BACKBONE_LINK_BW/1e9:.0f} Gbps
...
• Link utilization estimates"""
    ...
    def _initialize_simulation(self):
        ...
        self.simulation_thread = threading.Thread(
            target=run_proper_noc_simulation,
    ...
    def show_performance_summary(self):
        ...
        try:
            if PERF_TRACKER:
                ...
                summary_text = f"""Enhanced CHI Fat-Tree NoC Performance Summary
{"="*55}

SIMULATION PARAMETERS
Simulation Time: {sim_time:.6f} seconds
Topology: 2-Level Fat-Tree with realistic on-chip timing
Switch Processing Delay: {SWITCH_PROCESSING_DELAY*1e9:.1f} ns
Access Link Bandwidth: {ACCESS_LINK_BW/1e9:.0f} Gbps
Backbone Link Bandwidth: {BACKBONE_LINK_BW/1e9:.0f} Gbps
...
Backbone Link Utilization: {summary['backbone_link_utilization']*100:.4f}%
...
"""
    ...
    def run_writeback_test(self):
        """Test CORRECTED CHI writeback behavior"""
        LOGGER.log("Testing ENHANCED CHI writeback behavior...")
        LOGGER.log(" Step 1: GPU0 writes A5=100 (dirty in cache)")
        self.queue_command("write 0 5 100")

        self.after(2000, lambda: [
            LOGGER.log(" Step 2: GPU1 reads A5 (should trigger writeback from GPU0)"),
            self.queue_command("read 1 5")
        ])

        self.after(4000, lambda: [
            LOGGER.log(" Step 3: Check that memory now has value 100 from writeback"),
            LOGGER.log(" Both GPUs should have A5 in Shared state")
        ])

    def run_modified_writeback_test(self):
        """Test MODIFIED writeback behavior - no writeback when other GPU has dirty data"""
    ...
    def run_coherency_test(self):
        ...
        self.after(2000, lambda: self.queue_command("write 2 7 200")) # GPU2 writes A7 (invalidates others, writebacks)
